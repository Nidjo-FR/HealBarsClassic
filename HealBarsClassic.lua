...
HealBarsClassic.softMitStatusTextConfigList = {
	...
	['ICEFORT'] = 'Icebound Fortitude - ICEFORT',
	['SURVINST'] = 'Survival Instincts - SURVINST',
	['SACRI'] = 'Hand of Sacrifice - SACRI',
	['LASTSTD'] = 'Last Stand - LASTSTD'
}
HealBarsClassic.miscStatusTextConfigList = {
	['INERVTE'] = 'Innervate - INERVTE', 
	['SPIRIT'] = 'Spirit of Redemption - SPIRIT', 
	['ENRAGE'] = 'Enrage - ENRAGE', 
	['DIVSAC'] = 'Divine Sacrifice - DIVSAC', 
	['RECK'] = 'Recklessness - RECK', 
	['REFLECT'] = 'Spell Reflection - REFLECT', 
	['MISDIR'] = 'Misdirection - MISDIR', 
	['TRICKS'] = 'Tricks of the Trade - TRICKS'
}

local defensiveSpells = {
	[642] = {name = 'DIVSHLD',duration = 12, priority = 2} -- Divine Shield
	, [1022] = {name = 'BOP',duration = 6, priority = 10} -- Hand of Protection Rank 1
	, [5599] = {name = 'BOP',duration = 8, priority = 10} -- Hand of Protection Rank 2
	, [10278] = {name = 'BOP',duration = 10, priority = 10} -- Hand of Protection Rank 3
	, [45438] = {name = 'ICEBLCK',duration = 10, priority = 2} -- Ice Block
	, [19752] = {name = 'DIVINTR',duration = 45, priority = 1} -- Divine Intervention -- (Rarely held for full duration (180 sec) & frequently breaks from release->res)
	...
	, [31224] = {name = 'CLOAK', duration = 5, priority = 15} -- Cloak of Shadows
	, [47585] = {name = 'DISPERS', duration = 6, priority = 12} -- Dispersion
	
	, [498] = {name = 'DIVPROT',duration = 12, priority = 2} -- Divine Protection
	, [871] = {name = 'SHDWALL', duration = 12, priority = 5} -- Shield Wall	
	, [30823] = {name = 'SHMRAGE', duration = 15, priority = 16} -- Shamanistic Rage
	, [48792] = {name = 'ICEFORT', duration = 12, priority = 14} -- Icebound Fortitude	
	, [22812] = {name = 'BARKSKIN', duration = 12, priority = 30} -- Barkskin
	, [33206] = {name = 'PAINSPR', duration = 8, priority = 25} -- Pain Suppression
	, [61336] = {name = 'SURVINST', duration = 20, priority = 22} -- Survival Instincts
	, [6940] = {name = 'SACRI', duration = 12, priority = 19} -- Hand of Sacrifice
	, [12975] = {name = 'LASTSTD', duration = 20, priority = 23} -- Last Stand
	
	, [20711] = {name = 'SPIRIT', duration = 15, priority = 1} -- Spirit of Redemption
	, [29166] = {name = 'INERVTE', duration = 10, priority = 40} -- Innervate 
	, [5229] = {name = 'ENRAGE', duration = 10, priority = 39} -- Enrage 
	, [64205] = {name = 'DIVSAC', duration = 10, priority = 38} -- Divine Sacrifice 
	, [1719] = {name = 'RECK', duration = 4, priority = 37} -- Recklessness -- (Rarely held for full duration (12 sec))
	, [23920] = {name = 'REFLECT', duration = 2, priority = 36} -- Spell Reflection -- (Rarely held for full duration (5 sec))
	, [34477] = {name = 'MISDIR', duration = 6, priority = 35} -- Misdirection -- (Rarely held for full duration (30 sec))
	, [57934] = {name = 'TRICKS', duration = 6, priority = 34} -- Tricks of the Trade
	} 
local shieldSpells = {
...
function HealBarsClassic:HealComm_HealUpdated(event, casterGUID, spellID, healType, endTime, ...)
	...
	elseif HBCdb.global.timeframe < 4 and healType == libCHC.HOT_HEALS then 
		...
		--]]
	-- elseif healType == CHANNEL_HEALS then self:UpdateIncoming(nil, ...)
	else
...
--[[function HealBarsClassic:UpdateIncoming(callbackTime, ...)
	...
	for i=1, select("#", ...) do
		...
		if not HBCdb.global.seperateOwnColor then
			...
			if ownHealTime then
			    beforeHealAmount = (libCHC:GetTimeframeHealAmount(targetGUID,healType,currentTime, ownHealTime - 0.001, playerGUID) or 0)
				+ (libCHC:GetTimeframeHealAmount(targetGUID,channelType,currentTime, ownHealTime - 0.001, playerGUID) or 0)
